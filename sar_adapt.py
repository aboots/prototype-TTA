"""
SAR (Sharpness-Aware and Reliable entropy minimization) implementation.
Combined from SAR/sar.py and SAR/sam.py.
"""
...
class SAR(nn.Module):
    ...
    def __getattr__(self, name):
        """Forward attribute access to the underlying model if not found in SAR.
        This allows ProtoViT to access model attributes like prototype_class_identity.
        """
        try:
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    # ProtoViT returns (logits, min_distances, values) - extract logits for SAR
    if isinstance(model_output, tuple):
        outputs = model_output[0]  # logits
    else:
    ...
    model_output2 = model(x)
    if isinstance(model_output2, tuple):
        outputs2 = model_output2[0]  # logits
    else:
        outputs2 = model_output2
    ...
    return model_output, ema, reset_flag
