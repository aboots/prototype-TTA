...
#         # Only consider prototypes belonging to pseudo-label class
#         masked_sims = sim_scores * target_mask
        
#         # Normalize to get probabilities for each sample
#         proto_probs = masked_sims / (masked_sims.sum(dim=1, keepdim=True) + 1e-8)
        
#         # Binary entropy: minimize entropy to sharpen distribution
#         # H = -sum(p * log(p) + (1-p) * log(1-p))
#         # For sharpening, we minimize entropy
#         eps = 1e-8
#         entropy = -(proto_probs * torch.log(proto_probs + eps) + 
#                    (1 - proto_probs) * torch.log(1 - proto_probs + eps))
        
#         loss = entropy.sum(dim=1).mean()
...
class ProtoEntropy(nn.Module):
    ...
    def __getattr__(self, name):
        """Forward attribute access to the underlying model if not found in ProtoEntropy."""
        try:
            return super().__getattr__(name)
        except AttributeError:
            return getattr(self.model, name)
    ...
    def forward_and_adapt(self, x):
        ...
        # -----------------------------------------------------------
        # LOSS 3: Class-Conditional Evidence Alignment
        # -----------------------------------------------------------
